...
<div class="mt-[120px] mb-[200px] pb-[200px] flex justify-center">
	<div class="flex flex-row w-[1290px] flex-wrap gap-[30px]">
		{#each data.posts as post, index}
			{#if index === 0}
				<article class="w-[1290px] h-[554px] rounded-[10px] flex flex-row" style="box-shadow: 0 4px 50px rgba(0, 0, 0, .1);">
					<img
					...
					>
						...
						<a href={`/blogs/${post.slug}`} class="text-[36px] font-bold leading-[1.25] transition duration-300 hover:text-[#71b095]"
							>{post.title}</a
						>
			...
			{:else}
				<article class="w-[630px] h-[675px] mt-[20px] rounded-[5px]" style="box-shadow: 0 4px 50px rgba(0, 0, 0, .1);">
					<img class="w-[630px] h-[417px] object-cover rounded-tl-[5px] rounded-tr-[5px]" src={images[(index)]} alt="" />
					<div class="h-[258px] px-[40px] pt-[50px] pb-[45px]">
						<div class="mb-[25px] leading-[1.25] font-bold text-[16px]">
							{formatDate(post.created_at)}
						</div>
						<a class="text-[36px] leading-[1.25] font-bold hover:text-[#71b095] transition duration-300" href={`/blogs/${post.slug}`}>{post.title}</a>
					</div>
				</article>
			{/if}
		{/each}
